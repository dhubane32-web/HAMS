...
for (const [email, role] of accounts) {
  ...
  if (!roleOk) { ok = false; continue; }

  const dashRes = await fetch(`${base}/api/dashboard/${role}`, { headers: { authorization: `Bearer ${token}` } });
  console.log(`  dashboard/${role}=${dashRes.status}`);
  if (!dashRes.ok) ok = false;
}

for (const p of pages) {
  const r = await fetch(`http://localhost:3010${p}`, { method: 'GET', redirect: 'manual' });
